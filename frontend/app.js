...
function updateSimpleView() {
  ...
  // Select previously selected wallet
...
function updateSimpleTransactionList(walletId) {
  ...
  const ledger = loadLedger();
  const walletEntries = ledger.filter((entry) => entry.wallet_id === walletId);
...
function updateReceiveAddress(walletId) {
  ...
  // Use unified address if available, otherwise transparent
  const address =
...
function initViewModeUI() {
  ...
  // Simple view go to wallet tab link
